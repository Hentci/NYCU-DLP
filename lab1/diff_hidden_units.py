...
def plot_multiple_loss_curves(loss_histories, hidden_units_configs, title):
    plt.figure(figsize=(10, 5))
    for i, loss_history in enumerate(loss_histories):
        plt.plot(loss_history, label=f'hidden units={hidden_units_configs[i]}')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title(title)
    plt.legend()
...
hidden_units_configs = [(2, 2), (4, 4), (16, 16)]

loss_histories_linear = []

# 針對不同的hidden units配置進行訓練
for hidden_units in hidden_units_configs:
    hidden_size1, hidden_size2 = hidden_units
    print(f"Training with hidden units: {hidden_units}")
    ...
    show_result(X_linear, y_linear, pred_y_linear, title=f'Predict result ({hidden_units})')

plot_multiple_loss_curves(loss_histories_linear, hidden_units_configs, title="Loss Curves for Linear Data")
...
for hidden_units in hidden_units_configs:
    ...
    show_result(X_xor, y_xor, pred_y_xor, title=f'Predict result ({hidden_units})')

plot_multiple_loss_curves(loss_histories_xor, hidden_units_configs, title="Loss Curves for XOR Data")