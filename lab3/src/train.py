...
from utils import dice_score, train_loss, plot_accuracy
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_loader:
            ...
            train_dice_epoch += dice_score(outputs, masks).item() * images.size(0)
        
        train_loss_epoch /= len(train_loader.dataset)
        train_dice_epoch /= len(train_loader.dataset)
        train_losses.append(train_loss_epoch)
        train_accuracies.append(train_dice_epoch)
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                val_dice_epoch += dice_score(outputs, masks).item() * images.size(0)
        
        val_loss_epoch /= len(val_loader.dataset)
        val_dice_epoch /= len(val_loader.dataset)
        val_losses.append(val_loss_epoch)
        val_accuracies.append(val_dice_epoch)
        
        # Print epoch information including learning rate
        current_lr = optimizer.param_groups[0]['lr']
        print(f'Epoch {epoch+1}/{args.epochs}, Train Loss: {train_loss_epoch:.4f}, Val Loss: {val_loss_epoch:.4f}, Train Dice: {train_dice_epoch:.4f}, Val Dice: {val_dice_epoch:.4f}, Learning Rate: {current_lr:.6f}')
    ...
    train_loss(train_losses, val_losses, save_path='res34_loss_plot.png')
    
    # Plot and save the accuracy graph
    plot_accuracy(train_accuracies, val_accuracies, save_path='res34_accuracy_plot.png')
...
def get_args():
    ...
    parser.add_argument('--learning-rate', '-lr', type=float, default=5e-4, help='learning rate')
